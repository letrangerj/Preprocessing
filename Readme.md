...
This workflow consists of six main steps for processing and organizing microscopy images. Below is a detailed explanation of each step and the resulting file structure changes.
...
5. Prepare DAPI images for stitching (4_PrepareStitch.py)
6. Align and stitch images (5_Align_Batch.ijm)
...
### Step 5: Prepare DAPI Images (4_PrepareStitch.py)

This script prepares DAPI images for the stitching process by organizing them in a dedicated structure.

**Input:**

- Source directory containing processed images
- Parent folder path for creating the Stitch directory

**Output:**

- Organized DAPI images ready for stitching
...
### Step 6: Align and Stitch Images (5_Align_Batch.ijm)

This ImageJ macro performs automated alignment and stitching of DAPI images across different experimental rounds.

**Input:**

- Directory containing organized DAPI images (output from Step 5)
- Target directory for alignment results

**Output:**

- Stitched images
- TileConfiguration.registered.txt files containing alignment information
...
4. Run `3_Move_Formal.py` to organize the merged images for analysis.
5. Execute `4_PrepareStitch.py` to prepare DAPI images for stitching.
6. Use ImageJ to run the `5_Align_Batch.ijm` macro for aligning and stitching images.
...
- The stitching process requires consistent DAPI staining across all experimental rounds.
